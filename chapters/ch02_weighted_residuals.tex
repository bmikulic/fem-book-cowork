...
\begin{tikzpicture}[>=Stealth, x=1cm, y=1cm]
  ...
  %% Distributed axial load arrows -- same length, evenly spaced.
  %% Load magnitude is described by the equation label; the schematic
  %% only marks the load location and direction.
  \foreach \x in {0.55,1.25,...,5.45}{
    \draw[load] (\x,0.55) -- ++(0.55,0);
  }
