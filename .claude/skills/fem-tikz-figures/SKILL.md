...
5b. **Don't float a dim line, don't crowd it either.** A dim line must stay *visually attached* to the points it measures, but also stand off the edge enough that it doesn't crowd nearby arrows. Target ~10 mm of offset in print, which is `10 mm / x_scale_cm` TikZ units (≈14 units at `x=0.07cm`). Aim for the 12–18-unit band in scaled coords; less reads as cramped against force arrows or supports, more reads as floating. If avoiding label collisions would push past ~half the dimensioned span, switch to relocate-and-extend: (i) move the conflicting labels to free quadrants (e.g., `below right` / `above right` so they sit outside the dimensioned y-range), and (ii) keep the dim line in the visual band, adding short *extension lines* `\draw[thin] (geom_point) -- (dim_endpoint)` to bridge the gap. Force midway labels can move to the geometry-interior side (`left=2pt,midway`) to free the outer side for the dim annotation.
...
7. **Color convention.** Blue (`blue!70!black`, thick) for displacements/kinematics; red (`red!75!black`, ultra thick) for forces/statics; lighter blue with multiple short arrows for distributed loads; hatched walls (`pattern=north east lines`) for fixed supports. **Geometry is always black** — outer outlines, mesh element boundaries, dimension lines, **and coordinate axes (global *or* local)**. No accent colors on geometry "for clarity" or "to distinguish element boundaries"; if the rationalization is "this isn't a force or a displacement so the color rule doesn't apply," that rationalization is wrong — geometry is black. **Blue is exclusively for displacement DOFs, not for axes, frames, or distinguishing markers.** **Red is exclusively for forces, not for DOFs even when the figure has no actual forces.**
7a. **Show all DOFs as arrows, never as text labels.** Every node displayed in an element diagram shows its full DOF set as `disp`-style arrows (along +x, +y for 2D; +x for 1D; +x, +y, +θ for beam/frame). Don't substitute a text annotation like "$u_1, v_1$" because the node sits on top of the global axes — draw the arrows; they may visually overlay the axes for the first ~1 unit, that's fine. Mixed treatment (arrows at some nodes, text at others) reads as inconsistent and obscures the kinematic content of the figure.
...
| DOFs drawn red dashed (or any non-blue style) | Author swapped the disp/force colors | DOFs always solid blue (`disp`); red is reserved for forces |
| Local or global coordinate axes drawn in blue | Picked a "distinguishing" color from the displacement palette | Axes are black `\draw[->,thick]`; rely on labels (x, y, x̄, ȳ) to disambiguate |
| Some nodes show DOF arrows, other nodes have text-only labels | Author skipped arrows where they'd overlap with global axes | Draw arrows at every node — overlap with axes is acceptable as long as DOF labels are positioned distinctly |
| Element drawn as thin gray bar | Author treated bar as a "background" marker | Bar/element geometry is `\draw[thick]` black, same convention as plate outlines |
